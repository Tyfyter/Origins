...
namespace Origins.Graphics.Primitives {
	public class VertexRectangle {
		...
		public void DrawLit(Vector2 screenPos, Vector2 pos0, Vector2 pos1, Vector2 pos2, Vector2 pos3, Func<Vector2, Color> lighting = null) {
			lighting ??= static pos => Lighting.GetColor(pos.ToTileCoordinates());
			...
			vertices[0].Color = lighting(pos0);
			vertices[1].Color = lighting(pos1);
			vertices[2].Color = lighting(pos2);
			vertices[3].Color = lighting(pos3);
		...
		public void DrawWeirdFancyLighting(Rectangle rectangle, Color color, in Matrix matrix, in Matrix drawMatrix, bool tileCoords = false) {
			...
			vertices[2].TextureCoordinate = Vector2.Transform(rectangle.BottomLeft(), matrix);
			vertices[3].TextureCoordinate = Vector2.Transform(rectangle.BottomRight(), matrix);
