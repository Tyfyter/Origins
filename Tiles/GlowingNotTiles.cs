	...
	public class GlowingNotTiles : GlobalTile {
			...
			if (type == TileID.Cactus) {
				...
				if (treeType is IGlowingModTree glowingTree) {
					...
					if (tile.TileFrameX < 22 || tile.TileFrameY < 198) {
						...
						int treeStyle = 0;
						if (!WorldGen.GetCommonTreeFoliageData(i, j, 0, ref treeFrame, ref treeStyle, out _, out int topTextureFrameWidth, out int topTextureFrameHeight)) return;
						float windFactor = 0;
						if (tile.WallType == WallID.None) windFactor = Main.instance.TilesRenderer.GetWindCycle(i, j, Main.instance.TilesRenderer._treeWindCounter);
						Vector2 offset = new(Main.offScreenRange, Main.offScreenRange);
						Vector2 unscaledPosition = Main.Camera.UnscaledPosition;
						Vector2 position = new Vector2(i * 16 - (int)unscaledPosition.X + 8, j * 16 - (int)unscaledPosition.Y + 16) + offset;
						switch (tile.TileFrameX) {
							case 22: {
								if (!glowingTree.TopTexture.Exists) break;
							...
							case 44: {
								if (!glowingTree.BranchesTexture.Exists) break;
		...
		public override void ModifyLight(int i, int j, int type, ref float r, ref float g, ref float b) {
			if (OriginExtensions.GetTreeType(i, j) is IGlowingModTree glowingTree) {
				(r, g, b) = glowingTree.LightEmission(i, j);
			}
		}
	}
}
