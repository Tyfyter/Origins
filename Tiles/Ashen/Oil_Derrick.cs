	...
	public class Oil_Derrick : ModSpecialChest, IComplexMineDamageTile, IMultiTypeMultiTile {
		...
		class Sound : AEnvironmentSound {
			...
			public override void UpdateSound(Vector2 position) {
				...
				if (Main.tileFrame[type] == 0 && Main.tileFrameCounter[type] == 0) {
					SoundEngine.PlaySound(SoundID.Item108.WithPitch(-0.7f).WithVolume(0.25f), position);
					SoundEngine.PlaySound(SoundID.Item148.WithPitch(-0.5f).WithVolume(0.35f), position);
				}
			}
		}
