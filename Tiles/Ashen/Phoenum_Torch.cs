...
namespace Origins.Tiles.Ashen {
	public class Phoenum_Torch_Tile : OriginTile {
		...
		public override void SetStaticDefaults() {
			...
			// Assets
			if (!Main.dedServ) RequestIfExists("Origins/Tiles/Defiled/Bile_Torch_Tile_Flame", out flameTexture);
		}
		...
		public override float GetTorchLuck(Player player) => player.InModBiome<Ashen_Biome>().ToInt();
		...
		public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b) {
			...
			if (tile.TileFrameX < 66) {
				Vector3 light = Phoenum_Torch.Light;
				r = light.X;
				g = light.Y;
				b = light.Z;
			}
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch) {
			...
			int style = TileObjectData.GetTileStyle(Main.tile[i, j]);
			if (style == 1) // ExampleWaterTorch should be a bit greener.
				color.G = 255;
	...
	public class Phoenum_Torch : ModItem, ICustomWikiStat {
		...
		public override void SetDefaults() {
			...
			Item.DefaultToTorch(TileType<Phoenum_Torch_Tile>(), 0, false);
		...
		public override void AddRecipes() {
			...
			.AddIngredient(ItemType<Phoenum>())
