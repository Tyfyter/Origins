		...
		public class Journal_Entry_Snippet : WrappingTextSnippet {
			...
			public Journal_Entry_Snippet(string key, Color color) {
				...
				if (Journal_Registry.Entries.TryGetValue(key, out JournalEntry entry)) {
					...
					for (int j = 0; j < _snippets.Count; j++) if (j + 1 < _snippets.Count && _snippets[j].UniqueDraw(true, out Vector2 size, null)) {
							size.Y -= lineSpace;
							if (_snippets[j].GetType() != _snippets[j + 1].GetType()) while (size.Y > 0) {
									size.Y -= lineSpace;
