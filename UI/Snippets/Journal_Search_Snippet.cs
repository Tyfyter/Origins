		...
		public override bool UniqueDraw(bool justCheckingString, out Vector2 size, SpriteBatch spriteBatch, Vector2 position = default, Color color = default, float scale = 1) {
			...
			if (lastHovered > 0 || focused) {
				if (focused) {
					...
					if (Text != oldText && Main.InGameUI.CurrentState is Journal_UI_Open journalUI) journalUI.SetSearchResults(Text);
					if (Main.inputText.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Enter) || Main.inputText.IsKeyDown(Microsoft.Xna.Framework.Input.Keys.Escape)) focused = false;
				}
				...
				for (int i = (int)(dimensions.Width / FontAssets.MouseText.Value.MeasureString("_").X); i-- > 0;) builder.Append('_');
