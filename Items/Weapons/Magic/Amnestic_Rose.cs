...
namespace Origins.Items.Weapons.Magic {
	...
	public class Amnestic_Rose_Flower_P : ModProjectile {
		...
		static void Draw(Vector2 center, float size) {
			...
			.UseImage0(TextureAssets.Extra[ExtrasID.RainbowRodTrailShape])
			.Apply(null, [new("uAlphaMatrix0", new Vector4(1.5f, 0, 0, 0))]);
			const int verts = 64;
	...
	public class Amnestic_Rose_Buff : ModBuff {
		...
		public override void Update(NPC npc, ref int buffIndex) {
			...
			if (!(NPCID.Sets.ShouldBeCountedAsBoss[npc.type] || npc.boss) && !npc.buffImmune[Slow_Debuff.ID]) originGlobalNPC.slowDebuff = true;
