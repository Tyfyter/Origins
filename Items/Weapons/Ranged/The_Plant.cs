...
namespace Origins.Items.Weapons.Ranged {
	...
	public class The_Plant : ModItem, ICustomDrawItem, IOilableItem {
		...
		void OnPostUpdateMiscEffects(Player player) {
			if (++ammoTimer > Item.useAnimation * (this.OilApplied() ? 24 : 30) * CombinedHooks.TotalUseSpeedMultiplier(player, Item)) {
				ammoTimer = 0;
				int ammoGen = 3 + ammoCount / 40;
				this.ConsumeOil(ammoGen);
				ammoCount += ammoGen;
