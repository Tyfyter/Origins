...
namespace Origins.NPCs.MiscB.Shimmer_Construct {
	public class Shimmer_Construct : ModNPC {
		...
		readonly int[] previousStates = new int[5];
		...
		public static void SelectAIState(Shimmer_Construct boss, List<AIState> from) {
			WeightedRandom<int> states = new(Main.rand);
			for (int i = 0; i < from.Count; i++) {
				states.Add(from[i].Index, from[i].GetWeight(boss, boss.previousStates));
			}
		...
		public class PhaseOneIdleState : AIState {
			internal static List<AIState> aiStates = [];
		...
		public class SpawnCloudsState : AIState {
			...
			public override void DoAIState(Shimmer_Construct boss) {
				...
				Vector2 unfurlPos = targetPos - new Vector2(-35, 25) * 16;
				npc.SpawnProjectile(null,
					npc.Center,
					npc.DirectionTo(unfurlPos) * 7,
				...
				unfurlPos = targetPos - new Vector2(35, 25) * 16;
				npc.SpawnProjectile(null,
					npc.Center,
					npc.DirectionTo(targetPos - new Vector2(35, 25) * 16) * 7,
					ModContent.ProjectileType<Shimmer_Construct_Cloud_Ball>(),
					1,
					1,
					unfurlPos.X,
