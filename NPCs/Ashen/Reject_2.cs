...
namespace Origins.NPCs.Ashen {
	public class Reject_2 : Glowing_Mod_NPC, IWikiNPC, IAshenEnemy, IReplaceAITypeSounds {
		public Rectangle DrawRect => new(0, 0, 34, 46);
		public int AnimationFrames => 3;
		public int FrameDuration => 3;
		public static int PowerUpTime => 12;
		public override void SetStaticDefaults() {
			Main.npcFrameCount[NPC.type] = 6;
			NPCID.Sets.NPCBestiaryDrawOffset[Type] = NPCExtensions.BestiaryWalkLeft;
			GetInstance<Smog_Storm.SpawnRates>().AddSpawn(Type, Smog_Storm.SpawnRates.Reject_2);
		}
		...
		public override void FindFrame(int frameHeight) {
			if (NPC.ai[2] < PowerUpTime && !NPC.IsABestiaryIconDummy) {
				NPC.frame.Y = NPC.frame.Height * ((int)(NPC.ai[2] * 2) / PowerUpTime);
				return;
			}
			if (NPC.velocity.Y != 0) {
			...
			NPC.DoFrames(4, 2.., NPC.velocity.X * NPC.direction);
		}
