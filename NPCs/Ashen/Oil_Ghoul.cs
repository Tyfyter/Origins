...
namespace Origins.NPCs.Ashen {
	public class Oil_Ghoul : Glowing_Mod_NPC, IAshenEnemy, IWikiNPC, ICustomWikiStat {
		...
		public override void SetDefaults() {
			...
			SpawnModBiomes = [
				ModContent.GetInstance<Ashen_Underground_Desert>().Type
			];
		}
		public override void OnHitPlayer(Player target, Player.HurtInfo hurtInfo) {
			target.AddBuff(ModContent.BuffType<Oiled_Debuff>(), 36);
		}
		public new static float SpawnChance(NPCSpawnInfo spawnInfo) {
			if (!Main.hardMode) return 0;
			if (!spawnInfo.DesertCave) return 0;
			if (!spawnInfo.Player.InModBiome<Ashen_Biome>()) return 0;
			return Ashen_Biome.SpawnRates.Ghoul;
		}
