...
namespace Origins.NPCs.Defiled {
	public class Defiled_Mite : ModNPC, IDefiledEnemy {
		...
		public override void OnHitByItem(Player player, Item item, NPC.HitInfo hit, int damageDone) {
			...
			for (int i = Main.rand.Next(3); i-- > 0;) Gore.NewGore(NPC.GetSource_Death(), NPC.position + new Vector2(baseX + Main.rand.Next(halfWidth), Main.rand.Next(NPC.height)), new Vector2(hit.Knockback * player.direction, -0.1f * hit.Knockback), Mod.GetGoreSlot("Gores/NPCs/DF_Effect_Small" + Main.rand.Next(1, 4)));
		}
