...
namespace Origins.NPCs.Defiled.Boss {
	...
	public class Defiled_Amalgamation : Glowing_Mod_NPC, IDefiledEnemy, ICustomWikiStat, IJournalEntrySource, IOutlineDrawer, IMinions, IBossChecklistEntry, IBossTitleInfo {
		...
		public void SpawnWisp(NPC npc) {
			if (npc.GetSwarmNPC()?.isSwarmBoss ?? false) return;
		...
		public class Spawn : SpawnPool {
			...
			public override void SetStaticDefaults() {
				...
				AddSpawn<Defiled_Amalgamation>(spawnInfo => spawnInfo.PlayerFloorY < Main.worldSurface && Main.tile[spawnInfo.PlayerFloorX, spawnInfo.PlayerFloorY].WallType != OriginsWall.GetWallID<Defiled_Stone_Wall>(WallVersion.Natural) ? 99999999 : 0);
			}
